...
def world_model_loss(
    ...
    transition_sample_weight: torch.Tensor | None = None,
    uncertainty_weight: float = 0.0,
) -> Dict[str, torch.Tensor]:
    """Return the total loss and its components."""
    state_error = torch.mean((predicted_next - target_next) ** 2, dim=1)
    if transition_sample_weight is not None:
        state_error = state_error * transition_sample_weight
    ...
    if transition_sample_weight is not None:
        reward_error = reward_error * transition_sample_weight
...
def world_model_aux_loss(
    ...
    event_sample_weight: torch.Tensor | None = None,
    reward_sample_weight: torch.Tensor | None = None,
    transition_sample_weight: torch.Tensor | None = None,
    reward_weight: float = 1.0,
    ...
    if (
        ...
        if transition_sample_weight is not None:
            feature_error = feature_error * transition_sample_weight
    ...
    if target_event is not None and "event_logits" in outputs and event_weight > 0.0:
        ...
        if event_sample_weight is not None:
            event_error = event_error * event_sample_weight
